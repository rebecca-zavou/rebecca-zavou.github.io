...
HT_ErrorCode HT_OpenIndex(const char *fileName, int *indexDesc){
  // Ελέγχουμε αν υπάρχει διαθέσιμη θέση στον πίνακα.
  int i;
  ...
  // Ανοίγουμε το αρχείο και αποθηκεύσουμε το file descriptor στον πίνακα open_files.
  ...
  *indexDesc = i;  // Αποθηκεύουμε τον δείκτη του ανοιχτού αρχείου.
  ...
  // Φέρνουμε το πρώτο μπλοκ στον πίνακα.
  ...
  for(int i = 0; i < hashBlocks; i++) {
    ...
    for (int j = 4; j < BF_BLOCK_SIZE && l < info->hashTable_size; j += sizeof(int)) {
      if (openFiles[indexDesc].hashTable != NULL) {
        ...
        strncpy(data+4+j, (void *)(&hashArray.hashTable[l]), sizeof(int));
      }
      l++;
      count++;
    }
...
void HashTable_resize(int** hash_table, HT_Info *info) {
  ...
  *hash_table = new_hash_table;
  Print_Hash_Table(*hash_table, info);
}
